...
must_haves:
  truths:
    - "User can look up any material master view (Basic Data 1/2, Purchasing, MRP 1/2/3/4, Plant Data/Storage 1/2, Accounting 1/2, Sales, Classification) with key fields and their underlying tables (MARA/MARC/MARD/MBEW)"
    - "CORRECTION blocks warn about 7 commonly-wrong field locations: Valuation Class in Accounting 1 (not Basic Data), Price Control in Accounting 1 (not MRP), Reorder Point in MRP 1 (not Basic Data), Purchasing Group in MARC (not vendor), GR-Based IV in LFM1 (not PO type), Standard Price in MBEW (not Costing view), Batch Mgmt flag in Plant Data/Storage 1 (not Basic Data)"
    - "Vendor master documented with LFA1 + LFB1 + LFM1 fields — purchasing view (LFM1) has emphasis with key fields WAERS/WEBRE/ZTERM/INCO1/INCO2/MINBW"
    - "Purchasing info records (EINA/EINE) and source lists (EORD) documented as master data objects with key fields and T-codes"
    - "S/4HANA Business Partner callout placed at vendor master section header"
  artifacts:
    - path: "modules/mm/master-data.md"
      provides: "Complete MM master data reference: material master (all views), vendor master (LFA1/LFB1/LFM1), info records (EINA/EINE), source lists (EORD)"
      min_lines: 250
      contains: "MBEW"
    - path: "modules/mm/master-data.md"
    ...
    - path: "modules/mm/master-data.md"
      provides: "CORRECTION note blocks for commonly-wrong field locations"
      contains: "CORRECTION"
  key_links:
    - from: "modules/mm/master-data.md MBEW section"
      to: "Valuation Class CORRECTION"
      via: "CORRECTION block within Accounting 1 view section"
      pattern: "BKLAS|Valuation Class|CORRECTION"
    - from: "modules/mm/master-data.md LFM1 section"
      to: "GR-Based IV CORRECTION"
      via: "CORRECTION block about WEBRE field location"
      pattern: "WEBRE|GR-Based|CORRECTION"
    - from: "modules/mm/master-data.md vendor master header"
      to: "S/4HANA Business Partner note"
      via: "S/4 callout at section header"
      pattern: "Business Partner|BP|BUT000"
---

<objective>
Populate modules/mm/master-data.md with the complete MM master data reference: material master organized by view group with underlying tables (MARA/MARC/MARD/MBEW), vendor master (LFA1/LFB1/LFM1 with purchasing emphasis), purchasing info records (EINA/EINE), and source lists (EORD). Include 7 CORRECTION blocks for commonly-wrong field locations.

Purpose: Delivers MM-04 (master data). This file answers "what table/field is X in?" and "which view do I find field Y?" queries. The CORRECTION blocks are the highest-value differentiator — the material master has more field-location traps than any other SAP object. Vendor master is self-contained here (not requiring FI module lookup) per CONTEXT.md decision.

Output: modules/mm/master-data.md — fully populated, ~250+ lines, YAML frontmatter, per-view/table sections with key fields.
</objective>
...
<task type="auto">
  <name>Task 1: Write master-data.md — frontmatter, material master (all views with CORRECTION blocks)</name>
  <files>/Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md</files>
  <action>
Write /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md starting with YAML frontmatter, then the material master section covering all views.
...
| Storage Bin | Physical location reference | MARD-LGPBE | Simple bin assignment (not WM) |
  ...
  <verify>
Run: wc -l /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md
Expected: 150+ lines after Task 1

Run: grep -c "CORRECTION" /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md
Expected: 7 (all 7 CORRECTION blocks present)

Run: grep "BKLAS" /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md
Expected: Present in Accounting 1 section with CORRECTION 1
  </verify>
  <done>
- modules/mm/master-data.md has valid YAML frontmatter
- Material master has all views documented: Basic Data 1/2, Purchasing, MRP 1/2/3/4, Plant Data/Storage 1/2, Accounting 1/2, Sales (brief), Classification (brief), MARD
- 7 CORRECTION blocks embedded at the correct view sections
- Each view has key fields table with Field/Description/Table.Column/Notes
- Underlying table (MARA/MARC/MARD/MBEW) noted at each section header
  </done>
</task>

<task type="auto">
  <name>Task 2: Write master-data.md — vendor master, info records, source lists, then commit</name>
  <files>/Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md</files>
  <action>
Append to /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md the vendor master, purchasing info records, and source list sections.
...
git -C /Users/coryebert/Documents/SAPInformation add modules/mm/master-data.md
git -C /Users/coryebert/Documents/SAPInformation commit -m "feat(05-02): populate modules/mm/master-data.md with MM master data reference

- Material master: all views (Basic Data 1/2, Purchasing, MRP 1-4, Plant Data/Storage 1/2,
  Accounting 1/2, Sales brief, Classification brief, MARD) with key fields per table
- 7 CORRECTION blocks: Valuation Class=Acctg1, Price Control=Acctg1, Reorder Point=MRP1,
  Purch Group=MARC, GR-Based IV=LFM1, Std Price=MBEW, Batch Mgmt=Plant Data/Storage 1
- Vendor master: LFA1 (general) + LFB1 (company code/FI) + LFM1 (purchasing org/MM emphasis)
- Purchasing info records: EINA (header) + EINE (purchasing org data) with pricing fields
- Source lists: EORD with validity, fixed/blocked source, MRP indicator
- S/4HANA callout: Business Partner replaces vendor master in S/4"
  </action>
  <verify>
Run: wc -l /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md
Expected: 250+ lines
...
Run: grep "WEBRE" /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md
Expected: Present in LFM1 section with CORRECTION 5

Run: grep "Business Partner" /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md
Expected: S/4HANA callout at vendor master section

Run: grep "EORD\|EINE\|EINA" /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md
Expected: All three tables documented

Run: git -C /Users/coryebert/Documents/SAPInformation log --oneline -1
Expected: Commit message contains "05-02" and "master-data"
  </verify>
  <done>
- modules/mm/master-data.md has 250+ lines with all master data objects documented
- Material master: all views with key fields tables and underlying table headers
- 7 CORRECTION blocks embedded at correct view sections
- Vendor master: LFA1 + LFB1 + LFM1 with LFM1 purchasing emphasis (WAERS, WEBRE, ZTERM, INCO1/2, MINBW)
- CORRECTION 5 about WEBRE in LFM1 (not PO type) embedded in vendor section
- S/4HANA Business Partner callout at vendor master header
- Purchasing info records (EINA/EINE) and source lists (EORD) documented with key fields
...
1. Line count: `wc -l /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md` — expect 250+
2. CORRECTION blocks: `grep -c "CORRECTION" /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md` — expect 7
3. Material master views: `grep "^### " /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md` — expect Basic Data 1, Basic Data 2, Purchasing, MRP 1-4, Plant Data/Storage 1-2, Accounting 1-2, plus LFA1, LFB1, LFM1, EINA, EINE, EORD
4. Vendor master: `grep "LFM1" /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md` — multiple references
5. S/4 callout: `grep "Business Partner" /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md` — present
6. Frontmatter: `head -8 /Users/coryebert/Documents/SAPInformation/modules/mm/master-data.md` — valid YAML
</verification>

<success_criteria>
- modules/mm/master-data.md exists, is populated (not placeholder), has valid YAML frontmatter
- Material master covers all views with key fields and underlying table at each header
- 7 CORRECTION blocks present warning about commonly-wrong field locations
- Vendor master self-contained: LFA1 + LFB1 + LFM1 with purchasing emphasis
- Info records (EINA/EINE) and source lists (EORD) documented
- S/4HANA Business Partner callout at vendor master section header
- File is committed to git with descriptive commit message
</success_criteria>

<output>
After completion, create /Users/coryebert/Documents/SAPInformation/.planning/phases/05-mm-module-foundation/05-02-SUMMARY.md using the summary template at @/Users/coryebert/.claude/get-shit-done/templates/summary.md.

Key values for summary frontmatter:
- phase: 05-mm-module-foundation
- plan: 02
- files_created: [modules/mm/master-data.md]
- files_modified: []
- subsystem: mm-master-data
- affects: [06-mm-advanced, 11-cross-module-integration] (OBYC account determination references valuation class; P2P process references master data)
</output>
