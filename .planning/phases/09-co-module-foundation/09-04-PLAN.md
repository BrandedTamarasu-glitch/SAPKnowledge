...
must_haves:
  truths:
    - "User can follow the complete period-end cost allocation process (assessment/distribution) from review through execution with role annotations"
    - "Assessment vs distribution critical distinction documented inline: assessment uses secondary CE (cat 42, loses original CEs); distribution preserves original CEs"
    - "Internal order settlement process documented with settlement rule verification, test run, live execution, and receiver types"
    - "Product costing run documented: CK11N create → CK24 mark → CK24 release, with MBEW-STPRS update verification step"
    - "CK24 TWO-step (mark then release) is explicit — mark alone does NOT update standard price"
    - "Period-end CO closing sequence documented as numbered sequence: repost → overhead calc → assessment → distribution → settlement → actual price calc → lock period"
    - "Cost center planning process documented: KP06 primary costs, KP26 activity prices, KSPI planning profile"
    - "modules/co/CLAUDE.md updated with specific Read When guidance for all four Phase 9 content files"
  artifacts:
    - path: "modules/co/processes.md"
      provides: "CO process flows: period-end allocation, internal order settlement, product costing run, cost center planning, period-end closing sequence"
      min_lines: 200
      contains: "KSU5"
    - path: "modules/co/processes.md"
    ...
    - path: "modules/co/CLAUDE.md"
      provides: "Updated File Index with specific Read When guidance for Phase 9 content files"
      contains: "processes.md"
  key_links:
    - from: "modules/co/processes.md assessment process"
      to: "cost element category 42"
      via: "inline explanation: assessment replaces original CEs with secondary CE"
      pattern: "category 42|secondary cost element|original cost element"
    - from: "modules/co/processes.md CK24 step"
      to: "MBEW-STPRS update"
      via: "two-step mark then release with verification"
      pattern: "mark|release|MBEW-STPRS|standard price"
    - from: "modules/co/processes.md period-end sequence"
      to: "all CO period-end T-codes"
      via: "numbered sequence table: KB61→KGI2→KSU5→KSV5→KO88→CO88→KSII"
      pattern: "KB61|KGI2|KSU5|KSV5|KO88|KSII"
...
<objective>
Populate modules/co/processes.md with CO business process flows: period-end cost allocation (assessment/distribution), internal order settlement, product costing run, cost center planning, and period-end CO closing sequence — each as narrative plus summary table with role annotations. Then update modules/co/CLAUDE.md to accurately index all four Phase 9 content files with specific Read When guidance.

Purpose: Delivers CO-03. This file answers "what is the period-end CO process?" and "how does assessment work?" queries. Period-end processes (allocation, settlement, product costing) are where CO delivers 80% of its value — this is the highest-priority CO process documentation. The CLAUDE.md update ensures Claude routes queries to the right file within the CO module.

Output: modules/co/processes.md (process flows), updated modules/co/CLAUDE.md (accurate index).
</objective>
...
<task type="auto">
  <name>Task 1: Write processes.md — frontmatter, period-end allocation (assessment/distribution), internal order settlement, product costing run</name>
  <files>modules/co/processes.md</files>
  <action>
Write modules/co/processes.md starting with YAML frontmatter, then the three primary CO processes.
...
---
  ...
  <verify>
Run: wc -l modules/co/processes.md
Expected: 150+ lines after Task 1

Run: grep "category 42\|secondary cost element" modules/co/processes.md
Expected: Assessment/distribution distinction documented

Run: grep "MBEW-STPRS\|mark.*release\|Mark.*Release" modules/co/processes.md
Expected: CK24 two-step documented

Run: grep "COBRB\|settlement rule" modules/co/processes.md
Expected: Settlement rule verification step present
  </verify>
  <done>
- Period-end allocation (assessment/distribution): 6-step process with critical distinction table
- Internal order settlement: 6-step process with settlement receiver type reference
- Product costing run: 5-step process with CK24 mark/release two-step explanation
- Assessment vs distribution comparison table (CE on receiver, transparency, which costs, typical use)
- CK24 MBEW-STPRS update explicitly documented
- Summary tables with Step/Activity/T-code/Role/Output columns
  </done>
</task>

<task type="auto">
  <name>Task 2: Write processes.md — cost center planning, period-end closing sequence; update CLAUDE.md; commit</name>
  <files>modules/co/processes.md, modules/co/CLAUDE.md</files>
  <action>
Append to modules/co/processes.md the remaining processes, then update CLAUDE.md.
...
Now update modules/co/CLAUDE.md. Keep existing frontmatter. Replace all content below frontmatter with:
...
- **PCA Separate Ledger** — ECC 6 only: GLPCA/GLPCT tables; eliminated in S/4HANA (Universal Journal)

After updating CLAUDE.md, commit both files:
```
git add modules/co/processes.md modules/co/CLAUDE.md
git commit -m "feat(09-04): populate modules/co/processes.md and update CLAUDE.md

- Process 1: Period-End Cost Allocation (6 steps; assessment KSU5 vs distribution KSV5)
  Critical distinction table: secondary CE cat 42 vs preserved original CEs
- Process 2: Internal Order Settlement (6 steps; KO88 with receiver type reference)
  Settlement receiver types: CTR, ORD, KST, FXA, PSP, RKS
- Process 3: Product Costing Run (5 steps; CK11N→CK24 mark→CK24 release)
  CK24 two-step documented: mark ≠ release, MBEW-STPRS only on release
- Process 4: Cost Center Planning (5 steps; KP06, KP26, KB31N)
- Process 5: Period-End CO Closing Sequence (9-step table)
  Sequence: KB61→KGI2→KSU5→KSV5→KO88→CO88→KSII→1KEG→lock
  Timing: CO runs after FI and MM period-end
- CLAUDE.md: File Index with specific Read When guidance for all 4 content files
- Key Concepts: CA, cost elements, assessment/distribution, settlement, CK24, PCA"
```
  </action>
  <verify>
Run: wc -l modules/co/processes.md
...
Run: grep -c "^## " modules/co/processes.md
Expected: 5+ process sections plus CO Process Overview

Run: grep "category 42\|cat 42" modules/co/processes.md
Expected: Assessment/distribution distinction inline

Run: grep "MBEW-STPRS" modules/co/processes.md
Expected: CK24 two-step with MBEW reference

Run: grep "KB61.*KGI2\|repost.*overhead" modules/co/processes.md
Expected: Period-end sequence documented

Run: grep "processes.md\|master-data.md\|tcodes.md\|config-spro.md" modules/co/CLAUDE.md
Expected: All four files referenced in File Index

Run: git log --oneline -1
Expected: Commit message contains "09-04"
  </verify>
  <done>
- modules/co/processes.md has 200+ lines with 5 process flows documented
- Period-end allocation: 6 steps with assessment vs distribution comparison table
- Internal order settlement: 6 steps with settlement receiver type reference
- Product costing: 5 steps with CK24 mark/release two-step explicit
- Cost center planning: 5 steps with KP06/KP26/KB31N
- Period-end closing sequence: 9-step ordered table with dependencies
- Assessment vs distribution critical distinction inline
- CK24 mark ≠ release clearly documented
- modules/co/CLAUDE.md updated with specific Read When guidance and Key Concepts
- Git commit created
  </done>
</task>
...
1. processes.md line count: `wc -l modules/co/processes.md` — expect 200+
2. Process count: `grep "^## [0-9]" modules/co/processes.md | wc -l` — expect 5
3. Assessment/distribution: `grep "category 42\|cat 42" modules/co/processes.md` — present
4. CK24: `grep "MBEW-STPRS" modules/co/processes.md` — present with mark/release
5. Period-end sequence: `grep "KB61\|KGI2\|KSU5\|KSV5\|KO88\|KSII" modules/co/processes.md` — all present in sequence table
6. Summary tables: `grep "^| Step" modules/co/processes.md | wc -l` — expect 4+
7. CLAUDE.md: `grep "master-data.md\|processes.md" modules/co/CLAUDE.md` — both files in index
8. Validation: `python3 scripts/validate.py modules/co/processes.md` — should pass
</verification>

<success_criteria>
- modules/co/processes.md exists, is populated, has valid YAML frontmatter
- 5 process flows: period-end allocation, internal order settlement, product costing, cost center planning, period-end closing sequence
- Assessment vs distribution critical distinction documented with comparison table
- CK24 two-step (mark then release) explicitly documented with MBEW-STPRS verification
- Internal order settlement receiver types documented
- Period-end closing sequence as ordered table with dependencies
- All summary tables have Step/Activity/T-code/Role/Output columns
- modules/co/CLAUDE.md File Index updated with specific Read When guidance
- Both files committed to git
</success_criteria>

<output>
After completion, create .planning/phases/09-co-module-foundation/09-04-SUMMARY.md using the summary template at @/home/corye/.claude/get-shit-done/templates/summary.md.

Key values for summary frontmatter:
- phase: 09-co-module-foundation
- plan: 04
- files_created: [modules/co/processes.md]
- files_modified: [modules/co/CLAUDE.md]
