...
must_haves:
  truths:
    - "User can follow the complete standard Order-to-Cash process from order creation (VA01) through FI document posting with role annotations at each step"
    - "Document flow is explicit: VBAK/VBAP → LIKP/LIPS → VBRK/VBRP → BKPF/BSEG with table references at each step"
    - "Availability check (ATP) documented inline at order creation step — separate from credit check"
    - "Credit check documented inline at order creation step — separate from availability check"
    - "PGI is identified as posting within VL02N (movement type 601), not a separate transaction"
    - "VF01/VF04 billing creates FI document automatically: Dr Customer / Cr Revenue"
    - "Returns process (VA01 RE → VL01N return delivery → GR of returns → credit memo) documented as separate variant"
    - "Credit/debit memo requests documented: VA01 CR/DR → VF01 billing → FI posting"
    - "Cash sales, rush orders, consignment, and third-party processing each documented as brief process variants"
    - "modules/sd/CLAUDE.md updated with specific Read When guidance for all four Phase 7 content files"
  artifacts:
    - path: "modules/sd/processes.md"
      provides: "O2C process flows: standard, returns, credit/debit memos, cash sales, rush orders, consignment, third-party"
      min_lines: 250
      contains: "VF01"
    - path: "modules/sd/processes.md"
    ...
    - path: "modules/sd/CLAUDE.md"
      provides: "Updated File Index with specific Read When guidance for Phase 7 content files"
      contains: "processes.md"
  key_links:
    - from: "modules/sd/processes.md standard O2C"
      to: "document flow"
      via: "table references at each step (VBAK/VBAP → LIKP/LIPS → VBRK/VBRP)"
      pattern: "VBAK|LIKP|VBRK|document flow"
    - from: "modules/sd/processes.md PGI step"
      to: "FI posting"
      via: "movement type 601, Dr COGS / Cr Inventory"
      pattern: "601|COGS|PGI"
    - from: "modules/sd/processes.md billing step"
      to: "FI document"
      via: "Dr Customer / Cr Revenue automatic posting"
      pattern: "Dr Customer|Cr Revenue|FI document"
...
<objective>
Populate modules/sd/processes.md with the Order-to-Cash process flows: standard O2C (primary — most detailed), returns, credit/debit memos, cash sales, rush orders, consignment, and third-party processing — each as narrative plus summary table with role annotations and document flow table references. Then update modules/sd/CLAUDE.md to accurately index all four Phase 7 content files.

Purpose: Delivers SD-03. This file answers "what is the end-to-end process for sales?" and "how does the document flow work?" queries. The standard O2C process with ATP check, credit check, PGI posting, and billing-to-FI handoff is the highest-value section. The CLAUDE.md update ensures Claude routes queries to the right file within the SD module.

Output: modules/sd/processes.md (process flows), updated modules/sd/CLAUDE.md (accurate index).
</objective>
...
<task type="auto">
  <name>Task 1: Write processes.md — frontmatter, standard O2C process (8 steps with ATP, credit check, PGI, billing detail)</name>
  <files>modules/sd/processes.md</files>
  <action>
Write modules/sd/processes.md starting with YAML frontmatter, then the standard Order-to-Cash process.
...
| 6 | Receive customer payment | F-28/F.13 | AR Accountant | Payment cleared (see FI processes.md) |
  ...
  <verify>
Run: wc -l modules/sd/processes.md
Expected: 100+ lines after Task 1

Run: grep "VBAK\|LIKP\|VBRK\|BKPF" modules/sd/processes.md
Expected: All document flow tables referenced

Run: grep "ATP\|availability check\|Availability Check" modules/sd/processes.md
Expected: ATP documented inline at order step

Run: grep "credit check\|Credit Check\|VKM1" modules/sd/processes.md
Expected: Credit check documented as separate inline step
  </verify>
  <done>
- Standard O2C process documented with 6 main steps + 2 substeps (ATP, credit check)
- Document flow reference at top: VBAK/VBAP → LIKP/LIPS → VBRK/VBRP → BKPF/BSEG
- ATP check and credit check documented as independent inline steps
- PGI identified as VL02N posting (movement type 601, Dr COGS / Cr Inventory)
- Billing creates FI document: Dr Customer / Cr Revenue
- Payment cross-references FI processes.md
- Summary table with Step/Activity/T-code/Role/Output columns
  </done>
</task>

<task type="auto">
  <name>Task 2: Write processes.md — process variants (returns, credit/debit memos, cash sales, rush orders, consignment, third-party); update CLAUDE.md; commit</name>
  <files>modules/sd/processes.md, modules/sd/CLAUDE.md</files>
  <action>
Append to modules/sd/processes.md the six process variants, then update CLAUDE.md.
...
Now update modules/sd/CLAUDE.md. Keep existing frontmatter. Replace all content below frontmatter with:
...
```
git add modules/sd/processes.md modules/sd/CLAUDE.md
git commit -m "feat(07-04): populate modules/sd/processes.md and update CLAUDE.md

- Process 1: Standard O2C (8 steps; VA01→VL01N→VL02N PGI→VF01→FI doc)
  Full document flow: VBAK/VBAP→LIKP/LIPS→VBRK/VBRP→BKPF/BSEG
  ATP check and credit check as independent inline steps
  PGI posting: movement type 601, Dr COGS / Cr Inventory
  Billing posting: Dr Customer / Cr Revenue
- Process 2: Returns (VA01 RE→return delivery→GR 651→credit memo)
- Process 3: Credit/debit memos (CR/DR doc types→billing→FI)
- Process 4: Cash sales (CS — order+delivery+billing simultaneous)
- Process 5: Rush orders (RO — auto delivery at order save)
- Process 6: Consignment (KB/KE/KR/KA with movement types 631-634)
- Process 7: Third-party (TAS item cat, PR auto-created, vendor ships direct)
- CLAUDE.md: File Index with specific Read When guidance for all 4 content files
- Key Concepts: sales area, customer master, condition technique, copy control,
  document flow, item category determination"
```
  </action>
  <verify>
Run: wc -l modules/sd/processes.md
...
Run: grep -c "^## " modules/sd/processes.md
Expected: 7+ process sections plus Document Flow Reference

Run: grep "VBAK.*LIKP\|LIKP.*VBRK" modules/sd/processes.md
Expected: Document flow chain referenced

Run: grep "movement type 601\|601" modules/sd/processes.md
Expected: PGI movement type documented

Run: grep "consignment\|Consignment" modules/sd/processes.md
Expected: All 4 consignment variants (KB, KE, KR, KA)

Run: grep "third-party\|Third-Party\|TAS" modules/sd/processes.md
Expected: Third-party processing with TAS item category

Run: grep "processes.md\|master-data.md\|tcodes.md\|config-spro.md" modules/sd/CLAUDE.md
Expected: All four files referenced in File Index

Run: git log --oneline -1
Expected: Commit message contains "07-04"
  </verify>
  <done>
- modules/sd/processes.md has 250+ lines with 7 process variants documented
- Standard O2C: 6 main steps + 2 substeps with document flow table references
- ATP and credit check as independent inline steps
- PGI identified as VL02N (movement type 601, Dr COGS / Cr Inventory)
- Billing: Dr Customer / Cr Revenue with VBRK/VBRP + BKPF/BSEG creation
- Returns: VA01 RE → return delivery → GR 651 → credit memo
- Credit/debit memos: CR/DR document types through billing to FI
- Cash sales, rush orders, consignment (4 scenarios with movement types), third-party (TAS)
- modules/sd/CLAUDE.md updated with specific Read When guidance and Key Concepts
- Git commit created
  </done>
</task>
...
1. processes.md line count: `wc -l modules/sd/processes.md` — expect 250+
2. Process count: `grep "^## [0-9]" modules/sd/processes.md | wc -l` — expect 7
3. Document flow: `grep "VBAK\|VBAP\|LIKP\|LIPS\|VBRK\|VBRP" modules/sd/processes.md` — all present
4. Summary tables: `grep "^| Step" modules/sd/processes.md | wc -l` — expect 3+ (standard O2C, returns, consignment)
5. CLAUDE.md: `grep "master-data.md\|processes.md" modules/sd/CLAUDE.md` — both files in index
6. Validation: `python3 scripts/validate.py modules/sd/processes.md` — should pass
</verification>

<success_criteria>
- modules/sd/processes.md exists, is populated, has valid YAML frontmatter
- 7 process variants: standard O2C, returns, credit/debit memos, cash sales, rush orders, consignment, third-party
- Standard O2C has narrative + summary table with role annotations and document flow table references
- ATP and credit check documented as independent inline steps
- PGI correctly identified as VL02N posting (not separate T-code)
- Billing to FI handoff documented (Dr Customer / Cr Revenue)
- Consignment all 4 scenarios with movement types 631-634
- Third-party with TAS item category and PR auto-creation
- modules/sd/CLAUDE.md File Index updated with specific Read When guidance
- Both files committed to git
</success_criteria>

<output>
After completion, create .planning/phases/07-sd-module-foundation/07-04-SUMMARY.md using the summary template at @/home/corye/.claude/get-shit-done/templates/summary.md.

Key values for summary frontmatter:
- phase: 07-sd-module-foundation
- plan: 04
- files_created: [modules/sd/processes.md]
- files_modified: [modules/sd/CLAUDE.md]
